...
type
  ...
  TileInfo = object
    ...
    rotation: int                     # Unknown (-1) or 0, 1, 2, 3 × 90° counterclockwise.
...
const
  ...
  Axes = [Top: Vertical, Left: Horizontal, Bottom: Vertical, Right: Horizontal]
...
# Load tiles.
var tiles: Tiles    # Mapping tile id -> tile.
var num, id: int
for line in "p20.data".lines:
...
func rotate(borders: var Borders; angle: int) =
  ...
  ## Note that border directions are unchanged by rotation.
  if angle != 0:
...
func flip(borders: var Borders; axis: Axis) =
  ...
  ## Note that border directions are reversed by flipping.
  if axis == Horizontal:
...
var borderTable: BorderTable    # Mapping tile id -> borders.
var grid: Grid                  # Mapping tile id -> tile info.
...
type Image = seq[string]         # Image represented as a sequence of strings.
